const VERSION = "20250305-22:11"; // Updated with differential backup support
...
let isDifferentialBackupEnabled = true; // Default to enabled
let differentialBackupStats = {
  totalChunks: 0,
  changedChunks: 0,
  bytesTotal: 0,
  bytesSaved: 0,
...
function initializeLoggingState() {
  ...
  // Initialize differential backup settings
  const diffBackupSetting = localStorage.getItem("differential-backup-enabled");
  if (diffBackupSetting !== null) {
    isDifferentialBackupEnabled = diffBackupSetting === "true";
  } else {
    // Default to enabled for new installations
    localStorage.setItem("differential-backup-enabled", "true");
  }

  // Load previous backup metadata if differential backups are enabled
  if (isDifferentialBackupEnabled) {
    loadBackupMetadata();
  }

  logToConsole("info", `Differential backup ${isDifferentialBackupEnabled ? 'enabled' : 'disabled'}`);
}
...
function createSyncStatus() {
  ...
  Object.entries(savedPosition).forEach(([_, value]) => {
...
function updateSyncStatus() {
  setTimeout(() => {
    ...
    const getSyncStatus = () => {
      ...
      // Add differential backup indicator if enabled
      const diffIndicator = isDifferentialBackupEnabled ?
        `<span class="hint--bottom" aria-label="Differential backup enabled">🔄</span>` : "";

      return `<span class="sync-indicator">
        <span class="sync-dot" style="background-color: ${
          isSynced ? "#10B981" : "#EF4444"
        }"></span>
        <span>${isSynced ? "Synced" : "Not synced"} ${diffIndicator}</span>
      </span>`;
    };
...
async function backupToS3(options = {}) {
  if (isExportInProgress) {
  ...
  try {
    ...
    // Load backup metadata if differential backups are enabled
    if (isDifferentialBackupEnabled && !options.forceFull) {
      loadBackupMetadata();
    }

    // Export data with differential backup support
    data = await exportBackupData({
      forceFull: options.forceFull || false
    });

    // Check if we can skip the upload (no changes in differential mode)
    if (data._differential && data._differential.noChanges) {
      logToConsole("info", "No changes detected in differential backup, skipping upload");
      // Update stats for UI
      differentialBackupStats = data._differential.stats;
      differentialBackupStats.lastBackupTime = Date.now();

      // Still update the timestamps and status
      const currentTime = new Date().toLocaleString();
      ...
      if (element) {
        element.innerText = `Last sync done at ${currentTime} (no changes)`;
      }

      return true;
    }

    // Track if this is a differential backup for logging
    isDifferential = data._differential && data._differential.enabled;

    // Remove the differential metadata before encryption
    if (data._differential) {
      delete data._differential;
    }

    logToConsole("start", `Starting backup encryption (${isDifferential ? 'differential' : 'full'})`);
...
function openSyncModal() {
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="mt-2 bg-gray-100 px-3 py-2 rounded-lg border border-gray-200 dark:bg-zinc-800 dark:border-gray-600">
                                    <div class="flex items-center justify-between">
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-400">
                                            Differential Backup
                                            <button class="ml-1 text-blue-600 text-lg hint--top hint--rounded hint--large" aria-label="Differential backup only uploads data that has changed since the last backup, significantly reducing bandwidth and storage usage. When enabled, the system tracks changes to your data and only sends the modified portions to the cloud.">ⓘ</button>
                                        </label>
                                        <div class="flex items-center">
                                            <input type="checkbox" id="differential-backup-toggle" class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                                            <label for="differential-backup-toggle" class="ml-2 block text-sm text-gray-700 dark:text-gray-400">
                                                Enable differential backups
                                            </label>
                                        </div>
                                    </div>
                                    <div id="differential-stats" class="mt-2 text-xs text-gray-600 dark:text-gray-400 hidden">
                                        <div class="grid grid-cols-2 gap-2">
                                            <div>
                                                <span class="font-medium">Last backup:</span>
                                                <span id="diff-last-backup">Never</span>
                                            </div>
                                            <div>
                                                <span class="font-medium">Data chunks:</span>
                                                <span id="diff-chunks">0</span>
                                            </div>
                                            <div>
                                                <span class="font-medium">Changed chunks:</span>
                                                <span id="diff-changed">0</span>
                                            </div>
                                            <div>
                                                <span class="font-medium">Bandwidth saved:</span>
                                                <span id="diff-saved">0 bytes</span>
                                            </div>
                                        </div>
                                        <div class="mt-2">
                                            <button id="force-full-backup-btn" class="text-xs text-blue-600 hover:text-blue-800">
                                                Force full backup
                                            </button>
                                        </div>
                                    </div>
  ...
  // Setup differential backup UI
  const differentialBackupToggle = document.getElementById("differential-backup-toggle");
  const differentialStatsDiv = document.getElementById("differential-stats");
  const forceFullBackupBtn = document.getElementById("force-full-backup-btn");

  if (differentialBackupToggle) {
    // Load the current setting
    differentialBackupToggle.checked = isDifferentialBackupEnabled;

    // Show/hide stats based on current setting
    if (differentialStatsDiv) {
      differentialStatsDiv.classList.toggle("hidden", !isDifferentialBackupEnabled);
    }

    // Update stats display
    updateDifferentialBackupStats();

    // Add event listener for toggle
    differentialBackupToggle.addEventListener("change", (e) => {
      toggleDifferentialBackup(e.target.checked);
      if (differentialStatsDiv) {
        differentialStatsDiv.classList.toggle("hidden", !e.target.checked);
      }
    });
  }

  // Add event listener for force full backup button
  if (forceFullBackupBtn) {
    forceFullBackupBtn.addEventListener("click", async () => {
      if (isExportInProgress) {
      ...
      const shouldProceed = await showCustomAlert(
        "This will perform a full backup, ignoring any differential backup settings. Continue?",
        "Force Full Backup",
        [
          { text: "Cancel", primary: false },
          { text: "Proceed", primary: true },
      ...
      if (shouldProceed) {
        queueCloudOperation("force-full-backup", () => backupToS3({ forceFull: true }));
      }
    });
  }
}
...
function exportBackupData(options = {}) {
  return new Promise((resolve, reject) => {
    ...
    logToConsole("info", "Starting data export", {
      ...
      differential: isDifferentialBackupEnabled && !options.forceFull
    });
    ...
    request.onsuccess = function (event) {
      ...
      Promise.all([
        ...
        .then(async () => {
          ...
          // If differential backup is enabled and we're not forcing a full backup
          if (isDifferentialBackupEnabled && !options.forceFull) {
            try {
              // Load backup metadata if not already loaded
              if (!lastBackupMetadata) {
                loadBackupMetadata();
              }

              // Generate chunks and hashes for current data
              const currentChunks = await generateDataChunks(exportData);

              // Identify changed chunks
              const { stats } = identifyChangedChunks(currentChunks, lastBackupMetadata);

              // Update stats
              differentialBackupStats = stats;
              differentialBackupStats.lastBackupTime = Date.now();

              // Log differential backup stats
              logToConsole("info", "Differential backup stats", {
                totalChunks: stats.totalChunks,
                changedChunks: stats.changedChunks,
                changePercentage: stats.changePercentage.toFixed(2) + '%',
                bytesTotal: stats.bytesTotal,
                bytesSaved: stats.bytesSaved,
                savingsPercentage: ((stats.bytesSaved / stats.bytesTotal) * 100).toFixed(2) + '%'
              });

              // If there are no changes, we can return the original data
              if (stats.changedChunks === 0) {
                logToConsole("info", "No changes detected, skipping backup");
                resolve({
                  ...exportData,
                  _differential: {
                    noChanges: true,
              ...
              // Store the full metadata for future differential backups
              lastBackupMetadata = currentChunks;
              localStorage.setItem("last-backup-metadata", JSON.stringify({
                metadata: currentChunks.metadata,
                chunks: currentChunks.chunks
              }));

              // For the actual backup, we can either:
              // 1. Return only changed chunks (true differential)
              // 2. Return the full data (simpler approach)

              // For now, we'll use the simpler approach and return full data
              // but add metadata about the differential backup
              resolve({
                ...exportData,
                _differential: {
                  enabled: true,
              ...
              logToConsole("error", "Error in differential backup, falling back to full backup", error);
...
/**
 * Updates the differential backup stats UI
 */
function updateDifferentialBackupStats() {
  const lastBackupEl = document.getElementById("diff-last-backup");
  const chunksEl = document.getElementById("diff-chunks");
  const changedEl = document.getElementById("diff-changed");
  const savedEl = document.getElementById("diff-saved");

  if (!lastBackupEl || !chunksEl || !changedEl || !savedEl) {
  ...
  if (differentialBackupStats.lastBackupTime) {
    const date = new Date(differentialBackupStats.lastBackupTime);
    lastBackupEl.textContent = date.toLocaleString();
  } else if (lastBackupMetadata && lastBackupMetadata.metadata && lastBackupMetadata.metadata.timestamp) {
    const date = new Date(lastBackupMetadata.metadata.timestamp);
    lastBackupEl.textContent = date.toLocaleString();
  } else {
  ...
  // Update chunks count
  chunksEl.textContent = differentialBackupStats.totalChunks ||
    (lastBackupMetadata && lastBackupMetadata.metadata ?
      lastBackupMetadata.metadata.chunkCount : 0);

  // Update changed chunks count
  changedEl.textContent = differentialBackupStats.changedChunks || 0;

  // Update bandwidth saved
  savedEl.textContent = formatBytes(differentialBackupStats.bytesSaved || 0);
}
...
localStorage.setItem = function (key, ...args) {
  ...
  originalSetItem.apply(this, [key, ...args]);
};

// Differential backup functions

/**
 * Computes a hash for a given string using SHA-256
...
/**
 * Splits data into chunks and computes hashes for each chunk
 * @param {Object} data - The data object to process
 * @param {number} chunkSize - Size of each chunk in characters
 * @returns {Promise<Object>} - Object with chunks and their hashes
 */
async function generateDataChunks(data, chunkSize = 10000) {
  const result = {
    chunks: {},
    metadata: {
      totalSize: 0,
  ...
  for (const key in data.localStorage) {
    ...
    const keyValuePair = JSON.stringify({ key, value });
    const hash = await computeHash(keyValuePair);
    result.chunks[`ls:${key}`] = {
      hash,
      data: keyValuePair,
      size: keyValuePair.length
    };
    result.metadata.totalSize += keyValuePair.length;
    result.metadata.chunkCount++;
  }

  // Process indexedDB
  for (const key in data.indexedDB) {
    // For larger objects, split them into chunks
    const value = data.indexedDB[key];
    ...
    if (serialized.length <= chunkSize) {
      ...
      for (let i = 0; i < chunkCount; i++) {
        ...
        const end = Math.min((i + 1) * chunkSize, serialized.length);
        const chunk = serialized.substring(start, end);
        const chunkKey = `idb:${key}:chunk:${i}/${chunkCount}`;
        ...
        result.metadata.totalSize += chunk.length;
        result.metadata.chunkCount++;
      }
    }
  }
...
/**
 * Compares current data chunks with previous backup metadata
 * @param {Object} currentChunks - Current data chunks with hashes
 * @param {Object} previousMetadata - Previous backup metadata
 * @returns {Object} - Object containing changed chunks and stats
 */
function identifyChangedChunks(currentChunks, previousMetadata) {
  if (!previousMetadata || !previousMetadata.chunks) {
    logToConsole("info", "No previous backup metadata found, performing full backup");
    return {
      changedChunks: currentChunks.chunks,
      stats: {
        totalChunks: currentChunks.metadata.chunkCount,
        changedChunks: currentChunks.metadata.chunkCount,
        bytesTotal: currentChunks.metadata.totalSize,
        bytesSaved: 0,
  ...
  // Identify changed or new chunks
  for (const key in currentChunks.chunks) {
    const currentChunk = currentChunks.chunks[key];
    const previousChunk = previousMetadata.chunks[key];

    if (!previousChunk || previousChunk.hash !== currentChunk.hash) {
      changedChunks[key] = currentChunk;
      changedCount++;
      changedSize += currentChunk.size;
    }
  }
  ...
  const stats = {
    ...
    bytesTotal: currentChunks.metadata.totalSize,
    bytesSaved: currentChunks.metadata.totalSize - changedSize,
    changePercentage: (changedCount / currentChunks.metadata.chunkCount) * 100
  };

  return { changedChunks, stats };
}

/**
 * Reconstructs full data from differential backup and previous metadata
 * @param {Object} changedChunks - The changed chunks from differential backup
 * @param {Object} previousMetadata - Previous backup metadata with unchanged chunks
 * @returns {Object} - The reconstructed full data
 */
function reconstructDataFromDifferential(changedChunks, previousMetadata) {
  const fullChunks = { ...previousMetadata.chunks, ...changedChunks };
  const result = {
    localStorage: {},
    indexedDB: {}
  };

  // Process all chunks to reconstruct the data
  const chunksByParent = {};

  for (const key in fullChunks) {
    const chunk = fullChunks[key];

    if (key.startsWith('ls:')) {
      // Handle localStorage items
      try {
        const parsed = JSON.parse(chunk.data);
        result.localStorage[parsed.key] = parsed.value;
      } catch (error) {
        logToConsole("error", `Error parsing localStorage chunk ${key}:`, error);
      }
    } else if (key.startsWith('idb:')) {
      if (chunk.isChunk) {
        // This is a part of a larger object
        const parentKey = chunk.parentKey;
        if (!chunksByParent[parentKey]) {
          chunksByParent[parentKey] = [];
        }
        chunksByParent[parentKey].push(chunk);
      } else {
        // This is a complete indexedDB item
        try {
          const parsed = JSON.parse(chunk.data);
          result.indexedDB[parsed.key] = parsed.value;
        } catch (error) {
          logToConsole("error", `Error parsing indexedDB chunk ${key}:`, error);
        }
      }
    }
  }

  // Reconstruct chunked indexedDB items
  for (const parentKey in chunksByParent) {
    const chunks = chunksByParent[parentKey].sort((a, b) => a.chunkIndex - b.chunkIndex);
    let fullData = '';
    ...
    try {
      result.indexedDB[parentKey] = JSON.parse(fullData);
    } catch (error) {
      logToConsole("error", `Error reconstructing chunked indexedDB item ${parentKey}:`, error);
    }
  }
...
/**
 * Toggles differential backup mode
 * @param {boolean} enabled - Whether to enable differential backups
 */
function toggleDifferentialBackup(enabled) {
  isDifferentialBackupEnabled = enabled;
  localStorage.setItem("differential-backup-enabled", enabled.toString());
  logToConsole("info", `Differential backup ${enabled ? 'enabled' : 'disabled'}`);

  // Reset metadata if disabled
  if (!enabled) {
    lastBackupMetadata = null;
    localStorage.removeItem("last-backup-metadata");
    differentialBackupStats = {
      totalChunks: 0,
      changedChunks: 0,
      bytesTotal: 0,
      bytesSaved: 0,
...
/**
 * Loads the previous backup metadata from localStorage
 */
function loadBackupMetadata() {
  try {
    const savedMetadata = localStorage.getItem("last-backup-metadata");
    if (savedMetadata) {
      lastBackupMetadata = JSON.parse(savedMetadata);
      logToConsole("info", "Loaded previous backup metadata", {
        timestamp: new Date(lastBackupMetadata.metadata.timestamp).toLocaleString(),
        chunkCount: lastBackupMetadata.metadata.chunkCount
      });
    }

    // Load differential backup setting
    const diffBackupSetting = localStorage.getItem("differential-backup-enabled");
    if (diffBackupSetting !== null) {
      isDifferentialBackupEnabled = diffBackupSetting === "true";
    }
  } catch (error) {
    logToConsole("error", "Error loading backup metadata:", error);
    lastBackupMetadata = null;
  }
}
